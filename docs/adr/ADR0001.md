...
src/modules/<modulo>/ ├── domain/ # entidades y reglas de negocio puras, sin dependencias externas ├── application/ # casos de uso y puertos (interfaces) que el dominio necesita │ └── ports/ └── infrastructure/ # adaptadores concretos: HTTP (Express), persistencia (PostgreSQL), tiempo real (WebSocket) ├── http/ └── persistence/
